...
# Update packages
RUN apt-get update -y && apt-get upgrade -y && apt-get dist-upgrade -y && apt-get -y autoremove && apt-get clean
...
# Run Teamserver
RUN sh -c "/opt/Havoc/Teamserver/teamserver server --profile /opt/Havoc/Teamserver/profiles/havoc.yaotl -v --debug"

# Run Client
RUN sh -c "/opt/Havoc/Client/Havoc"
